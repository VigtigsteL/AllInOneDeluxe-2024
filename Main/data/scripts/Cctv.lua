...
-- gang sorting
local applicable   = { MinSec = true, Normal = true, MaxSec = true };
local categoryNames   = {'MinSec', 'Normal', 'MaxSec', '4', '5', '6'};
local gangCategory = { [2] = 3, [3] = 2, [4] = 1 }; -- [keys] = gang id; values = category id
...
function setTooltip()
    if this.SortsTarget then
        if this.SortsGang then
            this.Tooltip = {"PrisonerSorter_TargetNGangFound", (this.TargetFoundCount or 0), "X", (this.GangFoundCount or 0), "Y"}
        else
