...
function Go()
	...
	if this.Damage >= 1 or (at_office and has_cm) then
		this.Tooltip = {"dangerous_meeting__stats", this.bonus_check_count, "X", this.bonus_generated * each_bonus, "Y", this.spoiling_count, "Z"}
	end
...
function Tooltip_Eval(p)
	if not p.eval_time_first then return end
	...
	if p.eval_chance_lowest == 100 then
		...
		if p.bonus_count > 0 then
			if p.BoilingPoint_lowest < 100 then
				p.Tooltip = {"prisoner_evaluation_info__bonus__boiling", p.eval_chance_first, "F", p.eval_chance_lowest, "L", p.eval_chance_current, "C", p.eval_chance_improved, "I", p.bonus_count, "B", p.eval_days, "D", p.BoilingPoint_lowest, "A", BoilingPoint_current, "B"}
			else
				p.Tooltip = {"prisoner_evaluation_info__bonus__boiling_full", p.eval_chance_first, "F", p.eval_chance_lowest, "L", p.eval_chance_current, "C", p.eval_chance_improved, "I", p.bonus_count, "B", p.eval_days, "D"}
			end
		else
			if p.BoilingPoint_lowest < 100 then
				p.Tooltip = {"prisoner_evaluation_info__no_bonus__boiling", p.eval_chance_first, "F", p.eval_chance_lowest, "L", p.eval_chance_current, "C", p.eval_chance_improved, "I", p.eval_days, "D", p.BoilingPoint_lowest, "A", BoilingPoint_current, "B"}
			else
				p.Tooltip = {"prisoner_evaluation_info__no_bonus__boiling_full", p.eval_chance_first, "F", p.eval_chance_lowest, "L", p.eval_chance_current, "C", p.eval_chance_improved, "I", p.eval_days, "D"}
			end
		end
	end
end
